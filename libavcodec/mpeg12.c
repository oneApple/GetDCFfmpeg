...
{
    int qscale = get_bits(&s->gb, 5);  //很重要，解码器一直需要使用
    if (s->q_scale_type) {
    ...
    if (IS_INTRA(mb_type)) {
        ...
        if (s->codec_id == CODEC_ID_MPEG2VIDEO) {
            if(s->flags2 & CODEC_FLAG2_FAST){
                ...
                for(i=0;i<mb_block_count;i++) {  //表示宏块中块的个数，4：2：0 是6个块
                    if (mpeg2_decode_block_intra(s, *s->pblocks[i], i) < 0)  //处理I帧的块，好像到反量化之后，没有反DCT
                        return -1;
                }
            }
        } else {
                               ...
                               int n)
{
    ...
    /* DC coefficient */
    if (n < 4){
        quant_matrix = s->intra_matrix;  //量化矩阵
        component = 0;
    }else{
        quant_matrix = s->chroma_intra_matrix;
        component = (n&1) + 1;
    }
    diff = decode_dc(&s->gb, component);  //应该是差值
    if (diff >= 0xffff)
        return -1;
    dc = s->last_dc[component];
    dc += diff;
    //keym
   // WriteDCToFile(s->picture->pict_type,dc,s->mb_x,s->mb_y,s->mb_width,s->mb_height);
	//keym
    s->last_dc[component] = dc;
                             ...
                             const uint8_t **buf, int buf_size)
{
    ...
    /* extra slice info */
    while (get_bits1(&s->gb) != 0) {  //标记，指示是否有 intra_slice  reserved_bits ，共8位
        skip_bits(&s->gb, 8);
    }
    ...
    if(mb_y==0 && s->codec_tag == AV_RL32("SLIF")){
        ...
        for(;;) {
            int code = get_vlc2(&s->gb, mbincr_vlc.table, MBINCR_VLC_BITS, 2);  //返回的是什么？
            if (code < 0){
    ...
    if (avctx->hwaccel) {  //应该是跟硬件有关的，可能是看能不能硬件加速
    ...
    for(;;) {
        ...
        MPV_decode_mb(s, s->block);  //这里面好像有反DCT
        ...
        if (s->mb_skip_run == -1) {
            ...
            s->mb_skip_run = 0;
            for(;;) {
                int code = get_vlc2(&s->gb, mbincr_vlc.table, MBINCR_VLC_BITS, 2);  //vlc是可变长的编码，在这里就是哈夫曼编码
                if (code < 0){
                                 ...
                                 const uint8_t *buf, int buf_size)
{
    ...
    width = get_bits(&s->gb, 12);  //horizontal_size_value
    height = get_bits(&s->gb, 12);   //vertical_size_value
    if (width <= 0 || height <= 0)
        return -1;
...
static int mpeg_decode_frame(AVCodecContext *avctx,
                             ...
                             AVPacket *avpkt)
{
    const uint8_t *buf = avpkt->data;  //以后解码所用的数据都是从buffer中获得的，buffer是从之前读取的packet中获得的
    ...
    //keym
	//MpegEncContext *ss = &s->mpeg_enc_ctx;
    //if(ss->picture != 0)
	//{
	//	CreateFrameFile(ss->picture->pict_type);
	//}
	//CountFrameNum();
    //keym
                             ...
                             const uint8_t *buf, int buf_size)
{
    Mpeg1Context *s = avctx->priv_data;
    MpegEncContext *s2 = &s->mpeg_enc_ctx;
    const uint8_t *buf_ptr = buf;  //buf_ptr是缓冲区的指针，很重要
    ...
    for(;;) {
        ...
        case PICTURE_START_CODE:
            ...
            if(mpeg_decode_postinit(avctx) < 0){  //这个函数进行了一些赋值的操作，可能对下面的继续解码有用
